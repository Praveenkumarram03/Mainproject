...
function Cobol() {
  ...
  const [sinoCounter, setSinoCounter] = useState(1); // Counter for SINO

  // Simulating the uploader's name for "Last Modified By"
  ...
  const handleFileUpload = (event) => {
    const newFiles = Array.from(event.target.files).map(file => ({
      ...
      sino: sinoCounter, // Assign the current counter to SINO
    ...
    // Update the files and increment SINO counter
    setFiles(prevFiles => [...prevFiles, ...newFiles]);
    setSinoCounter(prevCounter => prevCounter + newFiles.length); // Increment SINO counter by the number of new files
  };
